...
def stage_chunk_to_hdfs(records: list[dict], run_id: int, batch_id: int,
                         chunk_id: int, hdfs_staged_dir: str) -> None:
    """Write one streaming chunk of records to HDFS under run_<run>/batch_<bid>_chunk_<cid>.tsv.

    Multiple chunks per batch land in the same staged dir; downstream engines
    (MR/Pig/Hive) read the directory recursively and don't care about chunk count.
    """
    tmp_path = f'/tmp/nasa_b{run_id}_{batch_id}_{chunk_id}.tsv'
    try:
        with open(tmp_path, 'w', encoding='utf-8') as f:
            for r in records:
                f.write(record_to_tsv(r) + '\n')

        hdfs_dest = (f'{hdfs_staged_dir}/run_{run_id}/'
                     f'batch_{batch_id:03d}_chunk_{chunk_id:05d}.tsv')
        subprocess.run(
